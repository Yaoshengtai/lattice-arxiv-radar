...
function ReportView({ paper, back, patchPaper, isPublic }: { paper: DashboardPaper; back: () => void; patchPaper: (id: string, update: Partial<PaperRecord>) => void; isPublic: boolean }) {
  ...
  return <div className="report-layout"><aside className="report-toc"><button className="back-link" onClick={back}><ArrowLeft size={16} /> Back to radar</button><span>IN THIS REPORT</span>{sections.map((section, index) => <a key={section} href={`#section-${index}`}>{String(index + 1).padStart(2, "0")} {section}</a>)}</aside><article className="report-page"><button className="mobile-report-back" onClick={back}><ArrowLeft size={16} /> Back to radar</button><div className="report-meta"><span>{paper.primaryCategory}</span><span>arXiv:{paper.id}</span><span>{new Date(paper.publishedAt).toLocaleDateString("en-US")}</span><span className="verified"><Check size={12} /> {report.report_level === "full" ? "Full report" : "Abstract-level report"}</span></div><h1>{paper.title}</h1><p className="report-authors">{paper.authors.join(", ")}</p><div className="report-actions"><a href={paper.abstractUrl} target="_blank" rel="noreferrer">Abstract <ArrowUpRight size={14} /></a><a href={paper.pdfUrl} target="_blank" rel="noreferrer">PDF <ArrowUpRight size={14} /></a><a href={paper.htmlUrl} target="_blank" rel="noreferrer">HTML <ArrowUpRight size={14} /></a><a href={paper.sourceUrl} target="_blank" rel="noreferrer">Source <ArrowUpRight size={14} /></a></div><div className="report-score-panel"><ScoreRing score={paper.score} /><div><strong>Profile fit: {paper.score}/100</strong><p>{paper.rationale}</p></div><Breakdown paper={paper} /></div>
    <ReportSection id="section-0" number="01" title="Executive summary"><p className="lead-copy">{report.executive_summary}</p></ReportSection><ReportSection id="section-1" number="02" title="Why it fits"><p>{report.why_it_fits}</p><div className="fit-pills">{paper.tags.map((tag) => <span key={tag}><Check size={13} /> {tag}</span>)}</div></ReportSection><ReportSection id="section-2" number="03" title="Problem formulation"><p>{report.problem_formulation}</p></ReportSection><ReportSection id="section-3" number="04" title="Key contributions"><ol className="contribution-list">{report.contributions.map((item) => <li key={item}>{item}</li>)}</ol></ReportSection><ReportSection id="section-4" number="05" title="Method"><p>{report.method}</p></ReportSection><ReportSection id="section-5" number="06" title="Key equations" accent><p>Original notation is kept separate from model interpretation. Provenance and confidence are shown for every expression.</p>{report.key_equations.map((equation) => <div className="equation-card" key={`${equation.label}-${equation.locator}`}><MathBlock latex={equation.latex} label={equation.label} /><div className="equation-content"><div><span>SYMBOL DEFINITIONS</span><p>{equation.symbol_definitions.join(" · ") || "Not explicitly specified in the paper"}</p></div><div><span>PLAIN-ENGLISH INTERPRETATION</span><p>{equation.interpretation}</p></div><div><span>ROLE IN THE METHOD</span><p>{equation.role}</p></div><div className="equation-foot"><span>{equation.locator}</span><span>{equation.provenance}</span><span>{Math.round(equation.extraction_confidence * 100)}% extraction</span><span>{Math.round(equation.interpretation_confidence * 100)}% interpretation</span></div></div></div>)}</ReportSection><ReportSection id="section-6" number="07" title="Experiments and results"><p>{report.experiments}</p></ReportSection><ReportSection id="section-7" number="08" title="Limitations"><ul>{report.limitations.map((item) => <li key={item}>{item}</li>)}</ul><h4>Reproducibility</h4><p>{report.reproducibility}</p>{report.warnings.length > 0 && <div className="evidence-note"><FileText size={16} /><span><strong>Extraction warnings</strong>{report.warnings.join(" · ")}</span></div>}</ReportSection><ReportSection id="section-8" number="09" title="Research ideas"><ol className="idea-list">{report.research_ideas.map((item) => <li key={item}><Sparkles size={16} /><span>{item}</span></li>)}</ol></ReportSection>{!isPublic && <><ReportSection id="section-9" number="10" title="Personal organization"><div className="notes-form"><label>Personal tags<input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="causal inference, read next" /></label><label>Personal note<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Write your own observations…" /></label><button className="primary-action" onClick={() => patchPaper(paper.id, { personalTags: tags.split(","), personalNote: note })}>Save notes</button></div></ReportSection><div className="report-feedback"><div><strong>Reading state</strong><span>These changes are stored in research-data/library.json.</span></div><button onClick={() => patchPaper(paper.id, { read: !paper.read })}><BookOpen size={17} /> {paper.read ? "Mark unread" : "Mark read"}</button><button className={paper.saved ? "saved" : ""} onClick={() => patchPaper(paper.id, { saved: !paper.saved })}><Bookmark size={17} fill={paper.saved ? "currentColor" : "none"} /> {paper.saved ? "Saved" : "Save"}</button></div></>}</article></div>;
}
